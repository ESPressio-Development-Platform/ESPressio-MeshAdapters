...
enum class MeshAdapterPolicyResolution : std::uint8_t {
    Resolved=0,
    Unsupported,
    Rejected,
    Malformed
...
constexpr Primitive::PrimitiveAdmissionDisposition ToPrimitiveAdmission(
    ...
    switch(disposition) {
        case A::Accepted: return P::TemporarilyUnavailable; // ownership transferred; exact family M1 arrives asynchronously
        case A::Busy:
...
/// <summary>
/// Bounded asynchronous Mesh-to-A2 admission bridge preserving exact destination-family M1 semantics.
/// </summary>
/// <remarks>
/// Mesh's receiver boundary is synchronous while A2 executes family admission on a T1 worker. The first Receive call
/// therefore transfers complete byte ownership to A2 and reports TemporarilyUnavailable, never false destination
/// admission. The exact A2 completion is retained under the authenticated Mesh occurrence key. When completion advances
/// AdmissionGeneration and wakes the Mesh owner, DeferredLocal retries the same occurrence; this bridge returns the
/// retained family M1 without enqueueing a duplicate A2 item, then releases the correlation slot.
///
/// Mesh membership incarnation is intentionally not converted to Primitive RuntimeIncarnationId. A2's stronger
/// ValidatedOriginalSemanticSource remains unset here; family wire/runtime validation establishes any stronger semantic
/// origin required by that family. No link, route or Mesh-authentication fact is upgraded into Primitive admission.
/// </remarks>
...
class MeshAdapterIngressBridge final : public Mesh::IPrimitiveReceiver {
    ...
    MeshAdapterPolicyBinding _policy{};
    MeshAdapterAdmissionWakeTarget _wake{};
    std::array<Entry,TMaximumCorrelations> _entries{};
    std::mutex _mutex{};
    ...
    static bool DecodeCorrelation(std::uint64_t correlation,std::size_t& slot,std::uint64_t& generation) noexcept {
        const auto encodedSlot=static_cast<std::uint16_t>(correlation&0xFFFFU);
        generation=(correlation>>16U)&GenerationMask;
        if(encodedSlot==0U||generation==0U) return false;
    ...
    static bool SameOccurrence(const Entry& entry,const Mesh::MeshReceiveContext& context) noexcept {
        return entry.Current!=State::Free&&entry.Source==context.Source&&
               entry.SourceIncarnation==context.SourceIncarnation&&entry.MessageId==context.DeliveryMessageId;
    }
    static std::uint64_t AddSaturating(std::uint64_t now,std::uint64_t duration) noexcept {
        return duration>std::numeric_limits<std::uint64_t>::max()-now
            ?std::numeric_limits<std::uint64_t>::max():now+duration;
    }
    void SweepExpiredLocked(std::uint64_t now) noexcept {
        for(auto& entry:_entries)
            if(entry.Current!=State::Free&&entry.ExpiryNanoseconds!=0U&&entry.ExpiryNanoseconds<=now) entry.Current=State::Free;
    }
    bool AllocateLocked(const Mesh::MeshReceiveContext& context,std::uint64_t expiry,std::size_t& slot,std::uint64_t& generation) noexcept {
        for(std::size_t i=0;i<_entries.size();++i){
            ...
            generation=entry.SlotGeneration+1U;
            if(generation==0U) continue;
            entry.Source=context.Source;entry.SourceIncarnation=context.SourceIncarnation;
            entry.MessageId=context.DeliveryMessageId;entry.SlotGeneration=generation;
            entry.ExpiryNanoseconds=expiry;entry.Completion=Primitive::PrimitiveAdmissionDisposition::Malformed;
            entry.Current=State::Pending;slot=i;return true;
        }
        return false;
    }
    ...
    void Complete(const Adapters::AdapterInboundCompletion& completion) noexcept {
        ...
        if(!DecodeCorrelation(completion.Correlation,slot,generation)) return;
        bool published=false;
        {
            std::lock_guard<std::mutex> lock(_mutex);
            auto& entry=_entries[slot];
            if(entry.Current!=State::Pending||entry.SlotGeneration!=generation) return;
            entry.Completion=completion.Admission;
            entry.Current=State::Completed;
            published=true;
        }
        if(!published) return;
...
public:
    MeshAdapterIngressBridge(TAdapterRuntime& runtime,MeshAdapterPolicyBinding policy,
                             MeshAdapterAdmissionWakeTarget wake={}) noexcept
    ...
    /// <summary>Current non-wrapping generation consumed by Mesh DeferredLocal wake-driven retry.</summary>
    std::uint64_t AdmissionGeneration() const noexcept { return _admissionGeneration.load(std::memory_order_acquire); }

    /// <summary>Returns the exact family owned by this bridge.</summary>
    Primitive::PrimitiveFamilyId Family() const noexcept { return _policy.Family; }

    /// <summary>Reclaims expired/stale correlation slots without polling or altering Mesh network Seen/Forwarded state.</summary>
    std::size_t Expire(std::uint64_t now=System::Clock::Monotonic().NowNanoseconds()) noexcept {
        ...
        for(auto& entry:_entries){
            if(entry.Current!=State::Free&&entry.ExpiryNanoseconds!=0U&&entry.ExpiryNanoseconds<=now){entry.Current=State::Free;++released;}
        }
        return released;
    }
    ...
    Primitive::PrimitiveAdmissionDisposition Receive(
        ...
        using P=Primitive::PrimitiveAdmissionDisposition;
        if(_runtime==nullptr||!_policy||!context.IsValid()||!payload.IsValid()||payload.Size==0U)
            return P::Malformed;
        ...
        const auto submitted=_runtime->AdmitTrustedInbound(
            ...
            Adapters::AdapterRouteToken{},familyPolicy,correlation,{this,&MeshAdapterIngressBridge::Complete});
        if(submitted==Adapters::AdapterSubmissionDisposition::Accepted) return P::TemporarilyUnavailable;
        ReleaseIfPending(slot,slotGeneration);
        return ToPrimitiveAdmission(submitted);
    }
};
