...
class MeshAdapterIngressBridge final : public Mesh::IPrimitiveReceiver {
    ...
    void SweepExpiredLocked(std::uint64_t now) noexcept {
        for(auto& entry:_entries) {
            if(entry.Current!=State::Free&&entry.ExpiryNanoseconds!=0U&&entry.ExpiryNanoseconds<=now)
                entry.Current=State::Free;
        }
    }
    bool AllocateLocked(const Mesh::MeshReceiveContext& context,std::uint64_t expiry,
        ...
        for(std::size_t i=0;i<_entries.size();++i){
            ...
            entry.SourceIncarnation=context.SourceIncarnation;
            entry.MessageId=context.DeliveryMessageId;
            entry.SlotGeneration=generation;
            entry.ExpiryNanoseconds=expiry;
            entry.Completion=Primitive::PrimitiveAdmissionDisposition::Malformed;
            entry.Current=State::Pending;
            slot=i;
            return true;
        }
        return false;
    ...
    void Complete(const Adapters::AdapterInboundCompletion& completion) noexcept {
        ...
        while(current!=std::numeric_limits<std::uint64_t>::max()&&
              !_admissionGeneration.compare_exchange_weak(
                  current,current+1U,std::memory_order_release,std::memory_order_relaxed)) {}
...
public:
    ...
    std::uint64_t AdmissionGeneration() const noexcept {
        return _admissionGeneration.load(std::memory_order_acquire);
    }
    ...
    Primitive::PrimitiveAdmissionDisposition Receive(
        ...
        std::uint64_t slotGeneration=0;
        {
            ...
            for(auto& entry:_entries){
                ...
                const auto result=entry.Completion;
                entry.Current=State::Free;
                return result;
            }
