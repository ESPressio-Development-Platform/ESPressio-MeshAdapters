...
struct TestState final:S::TransmissibleState<TestState,Value>{
    ...
    static constexpr std::string_view CanonicalName="MeshAdapters.State.Outbound";
...
template<class P>void Eventually(P&& predicate){
...
struct RouteResolver final {
    ...
    static bool Resolve(void*,const System::DeviceIdentifier& device,Adapters::AdapterRouteToken& route) noexcept{
        if(!device)return false;route.Value=0xD000U+device.Bytes().back();return true;
    }
    static bool Broadcast(void*,Adapters::AdapterRouteToken&) noexcept{return false;}
};
struct ServiceSignal final {
    std::atomic<unsigned> Wakes{0};
    static void Wake(void* owner) noexcept{++static_cast<ServiceSignal*>(owner)->Wakes;}
};
...
struct Lower final {
    ...
    static Adapters::LowerTransportSubmitResult Submit(void* owner,Adapters::AdapterRecordIdentity,
        Adapters::AdapterServiceClass service,Adapters::AdapterByteView bytes,Adapters::AdapterRouteToken route) noexcept{
        auto& self=*static_cast<Lower*>(owner);assert(bytes.Data&&bytes.Size<=self.Wire.size());
        std::memcpy(self.Wire.data(),bytes.Data,bytes.Size);self.Bytes=bytes.Size;self.Route=route;self.Service=service;
        const auto generation=++self.Calls;return {Adapters::LowerTransportDisposition::PermanentlyRejected,generation,false};
    }
    Adapters::LowerTransportBinding Binding() noexcept{
        Adapters::LowerTransportBinding b{};b.Owner=this;b.Submit=&Lower::Submit;b.Validate=&Lower::Validate;b.ServiceClassMask=0x3f;
        b.ProvidesDestinationPrimitiveAdmission=true;b.ProvidesValidatedOriginalSource=true;return b;
    }
};
}
int main(){
    HostRuntime platform;
    const auto local=Identity(1,11),remote=Identity(2,22);
    ...
    A2 adapter;RouteResolver routes;ServiceSignal signal;
    ...
    MeshAdapters::StateMeshAdapterFamilyBinding<Runtime,1,2> family;
    assert((family.BindType<TestState,Serializable::DirectBinary>(runtime)==MeshAdapters::StateMeshAdapterBindingStatus::Success));
    assert(family.BindAdapter(adapter,routeBinding,{&signal,&ServiceSignal::Wake})==MeshAdapters::StateMeshAdapterBindingStatus::Success);
    ...
    Task::TaskExecutionConfiguration worker{};worker.Name="stateA2";worker.StackSize=4096;
    ...
    const auto wakesBeforeService=signal.Wakes.load();
    assert(family.Service());
    Eventually([&]{return lower.Calls.load()>=1;});
    assert(lower.Route.Value==0xD002U&&lower.Service==Adapters::AdapterServiceClass::Convergent);
    S::StatePublicationWireHeader header{};
    assert(S::DecodeStatePublicationHeader(lower.Wire.data(),lower.Bytes,header));
    assert(header.TypeId==TestState::TypeId&&header.Owner==local&&header.Requester==remote&&header.Session==session);
    Value value{};
    const auto decoded=S::DecodeStatePublicationValue<TestState,Serializable::DirectBinary>(header,
        lower.Wire.data()+S::StatePublicationWireHeaderSize,lower.Bytes-S::StatePublicationWireHeaderSize,value);
    assert(decoded&&value.Number==20);
    Eventually([&]{return signal.Wakes.load()>wakesBeforeService;});
    assert(family.Service());
