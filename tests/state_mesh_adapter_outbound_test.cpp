...
struct RouteResolver final {
    ...
    static bool Resolve(
        void*,const System::DeviceIdentifier& device,Adapters::AdapterRouteToken& route) noexcept {
...
using Inbound=Adapters::CapacityPlane<
    Adapters::AdapterDirection::Inbound,Domain,Domain,Domain,Domain,Domain,Domain,Domain,Domain>;
using Outbound=Adapters::CapacityPlane<
    Adapters::AdapterDirection::Outbound,Domain,Domain,Domain,Domain,Domain,Domain,Domain,Domain>;
...
struct Lower final {
    ...
    static Adapters::LowerTransportSubmitResult Submit(
        ...
        auto& self=*static_cast<Lower*>(owner);
        assert(bytes.Data&&bytes.Size<=self.Wire.size());
        std::memcpy(self.Wire.data(),bytes.Data,bytes.Size);
    ...
    Adapters::LowerTransportBinding Binding() noexcept {
        ...
        binding.Owner=this;
        binding.Submit=&Lower::Submit;
        binding.Validate=&Lower::Validate;
...
int main() {
    ...
    const auto remote=Identity(2,22);
    ...
    using Runtime=S::Runtime<
        S::TypeConfiguration<TestState,S::MaximumRemoteOwners<1>,S::MaximumSubscribers<1>>>;
    Runtime runtime;
    auto owner=runtime.BindOwner<TestState>();
    assert(owner);
    ...
    assert((family.BindType<TestState,Serializable::DirectBinary>(runtime)==
            MeshAdapters::StateMeshAdapterBindingStatus::Success));
    assert(family.BindAdapter(adapter,routeBinding,{&signal,&ServiceSignal::Wake})==
           MeshAdapters::StateMeshAdapterBindingStatus::Success);
    ...
    assert(runtime.ActivateSourceSubscriber<TestState>(
        remote,session,true,runtime.Version<TestState>())==S::StateRemoteStatus::Success);
    ...
    assert(lower.Route.Value==0xD002U);
    assert(lower.Service==Adapters::AdapterServiceClass::Convergent);

    S::StatePublicationWireHeader header{};
    assert(S::DecodeStatePublicationHeader(lower.Wire.data(),lower.Bytes,header));
    assert(header.TypeId==TestState::TypeId);
    assert(header.Owner==local);
    assert(header.Requester==remote);
    assert(header.Session==session);

    Value value{};
    const auto decoded=S::DecodeStatePublicationValue<TestState,Serializable::DirectBinary>(
        header,
        lower.Wire.data()+S::StatePublicationWireHeaderSize,
        lower.Bytes-S::StatePublicationWireHeaderSize,
        value);
    assert(decoded&&value.Number==20);

    Eventually([&]{ return signal.Wakes.load()>wakesBeforeService; });
    assert(family.Service());
    assert(runtime.NeedsConvergence<TestState>(
        remote.Device,S::StateContinuitySide::SourceSubscriber));
