...
struct RecoveryCommand final : C::TransmissibleCommand<RecoveryCommand,RecoveryResponse> {
    static constexpr C::CommandTypeId TypeId{0x4203};
    static constexpr std::string_view CanonicalName="MeshAdapters.Command.RecoveredResponse";
...
struct LowerTransport final {
    ...
    Adapters::LowerTransportBinding Binding() noexcept {
        ...
        binding.Owner=this;binding.Submit=&LowerTransport::Submit;binding.Validate=&LowerTransport::Validate;
...
Task::TaskExecutorConfiguration RouterConfiguration() {
    ...
    configuration.Execution.Name="meshCmdRecoveryRouter";
...
int main() {
    ...
    const auto ledgerKey=StoreKey("mesh-recovery-ledger");
    {
    ...
    configuration.ExecutionLane.Name="meshCmdRecovery";
    ...
    // Runtime initialization must reserve the recovered response route before MeshAdapter freeze/A2 start.
    ...
    assert(lower.Route.Value==0xE003U);
