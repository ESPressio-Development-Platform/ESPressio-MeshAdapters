...
struct MockAdapterRuntime final {
    ...
    Adapters::AdapterSubmissionDisposition CompleteTransport(const Adapters::LowerTransportCompletion& completion) noexcept {
        Last=completion;++CompletionCount;return Adapters::AdapterSubmissionDisposition::Accepted;
    }
};
...
int main(){
    MockAdapterRuntime runtime;MeshLifecycle lifecycle;
    MeshAdapters::MeshApplicationLifecycleBinding mesh{};
    mesh.Owner=&lifecycle;mesh.Submit=&MeshLifecycle::Submit;mesh.Validate=&MeshLifecycle::Validate;
    mesh.Cancel=&MeshLifecycle::Cancel;mesh.Quiesce=&MeshLifecycle::Quiesce;
    mesh.ServiceClassMask=static_cast<std::uint8_t>(ServiceBit(Mesh::MeshRelayServiceClass::Clock)|ServiceBit(Mesh::MeshRelayServiceClass::Responsive));
    mesh.ProvidesDestinationPrimitiveAdmission=true;mesh.ProvidesValidatedOriginalSource=true;
    MeshAdapters::MeshLowerTransportBinding<MockAdapterRuntime> transport(runtime,mesh);
    assert(transport.IsValid());auto binding=transport.AdapterBinding();assert(binding);
    assert(binding.ProvidesDestinationPrimitiveAdmission&&binding.ProvidesValidatedOriginalSource);
    assert(binding.Supports(Adapters::AdapterServiceClass::Clock));assert(binding.Supports(Adapters::AdapterServiceClass::Responsive));
    assert(!binding.Supports(Adapters::AdapterServiceClass::Critical));assert(binding.Validate(binding.Owner));
    ...
    lifecycle.Completion.Complete(lifecycle.Completion.Owner,record,submitted.Generation,Adapters::LowerTransportDisposition::Accepted,
        Primitive::PrimitiveAdmissionDisposition::AlreadyAccepted,true);
    assert(runtime.CompletionCount==1);assert(runtime.Last.Record.Slot==record.Slot);assert(runtime.Last.Record.Generation==record.Generation);
    assert(runtime.Last.TransportGeneration==17);assert(runtime.Last.Disposition==Adapters::LowerTransportDisposition::Accepted);
    assert(runtime.Last.HasDestinationAdmission);assert(runtime.Last.DestinationAdmission==Primitive::PrimitiveAdmissionDisposition::AlreadyAccepted);

    binding.Cancel(binding.Owner,record);binding.Quiesce(binding.Owner);assert(lifecycle.Cancelled&&lifecycle.Quiesced);
    lifecycle.Running=false;assert(!binding.Validate(binding.Owner));
    Mesh::MeshRelayServiceClass mapped{};
    assert(MeshAdapters::ToMeshRelayServiceClass(Adapters::AdapterServiceClass::Infrastructure,mapped));assert(mapped==Mesh::MeshRelayServiceClass::Infrastructure);
    assert(MeshAdapters::ToMeshRelayServiceClass(Adapters::AdapterServiceClass::BestEffort,mapped));assert(mapped==Mesh::MeshRelayServiceClass::BestEffort);
    return 0;
}
