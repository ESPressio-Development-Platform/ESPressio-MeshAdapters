...
struct QueryCommand final : C::TransmissibleCommand<QueryCommand,Reply> {
    static constexpr C::CommandTypeId TypeId{0x4202};
    static constexpr std::string_view CanonicalName="MeshAdapters.Command.DeliveryFailure";
...
struct Owner final {
    ...
    Reply Handle(const QueryCommand& command,const C::CommandExecutionContext&) { return {command.Value+1}; }
...
struct RouteResolver final {
    ...
    static bool Resolve(void*,const System::DeviceIdentifier& device,Adapters::AdapterRouteToken& route) noexcept {
        ...
        route.Value=0xC000U+device.Bytes().back();
        return true;
    }
...
Task::TaskExecutorConfiguration RouterConfiguration() {
    ...
    configuration.Execution.Name="meshCmdFailRouter";
...
int main() {
    ...
    const auto remote=Device(9);
    assert(System::RuntimeIdentity::Install({local,System::RuntimeIncarnationId{17}})==
    ...
    assert((family.ConfigureType<QueryCommand,Serializable::DirectBinary>(
                Mesh::MeshRelayServiceClass::Responsive,Mesh::MeshRelayServiceClass::Responsive)==
            MeshAdapters::CommandMeshAdapterBindingStatus::Success));

    QueryOutboundOwner<Family> outboundOwner{&family};
    C::CommandOutboundBinding<QueryCommand,Serializable::DirectBinary> outbound;
    assert((outbound.Initialize<QueryOutboundOwner<Family>,
        &QueryOutboundOwner<Family>::Admit,
        &QueryOutboundOwner<Family>::Validate,
        &QueryOutboundOwner<Family>::ReserveRecovered,
        &QueryOutboundOwner<Family>::ReleaseRecovered>(outboundOwner)));

    Store<4096,4> store;
    C::CommandResponseRouter<2> router(RouterConfiguration());
    C::RuntimeConfiguration commandConfiguration{};
    commandConfiguration.ExecutionLane.Name="meshCmdFail";
    ...
    assert(commandRuntime.BindPersistence<QueryCommand>(store,StoreKey("mesh-command-failure"))==C::CommandRuntimeStatus::Success);
    assert((commandRuntime.BindTransport<QueryCommand,Serializable::DirectBinary>(outbound)==
    ...
    const auto submitted=client.ExecuteTo<QueryCommand,&Owner::OnResult>(
        remote,std::chrono::milliseconds(500),73U);
    assert(submitted.Accepted());
    assert(client.IsLive(submitted.Request));
    Eventually([&]{ return lower.Calls.load()>=1; });
    assert(lower.Route.Value==0xC009U);

    C::CommandRequestWireHeader header{};
    assert(C::DecodeCommandRequestHeader(lower.Last.data(),lower.Bytes,header));
    assert(header.Key==submitted.Request.Key());
    assert(header.Key.TypeId==QueryCommand::TypeId);
    assert(header.Key.OriginDevice==local);
    assert(header.Key.OriginRuntime==System::RuntimeIncarnationId{17});

    Eventually([&]{ return client.IsReady(submitted.Request); });
    responses.Service({host.Now.load(),services});
    assert(owner.Callbacks.load()==1);
